...
int calcProc(void)
{/*calcProc为计算器实质过程，非负一位整数四则运算计算器*/
	char *in = NULL;
...
{/*将print(5号)中断的中断程序入口代码替换为ProcAddr，并返回原中断程序入口地址*/
	char far *foreProc = NULL;
	struct address
	{
		char far *ProcAddr;
	};
	struct address far *addr = (struct address far *)20;/*地址20指向中断向量表中第5个中断向量*/
	foreProc = addr->ProcAddr;/*foreProc保存5号中断原中断程序入口地址*/
	addr->ProcAddr = ProcAddress;/*修改IP为ProcAddress*/
	
	return foreProc;
}
int ResumePrtInterrupt(char far *foreAddr)
{/*恢复print(5号)中断的中断程序入口代码*/
	struct address
	{
		char far *ProcAddr;
	};
